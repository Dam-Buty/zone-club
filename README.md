...
**PWA.** Service worker à deux vitesses : cache-first sur les assets 3D, qui ne changent
qu'entre deux déploiements, et network-only sur tout le reste. Le catalogue est
délibérément du second groupe — c'est de la donnée mutable, sa fraîcheur revient au cache
HTTP du navigateur plutôt qu'à une invalidation applicative. Notifications push Web Push,
dont celle qui prévient que le film est terminé sur la télé pendant que le téléphone
dormait — et son clic rouvre l'app au bon endroit.
