...
import { filmListColumns, parseFilmListRow } from '@/lib/films';
...
export async function GET() {
    try {
        ...
        const returned = db.prepare(`
            SELECT ${filmListColumns('f')} FROM films f WHERE f.id IN (
        ...
        if (existingIds.length > 0) {
            ...
            random = db.prepare(`
                SELECT ${filmListColumns()} FROM films
                WHERE is_available = 1 AND id NOT IN (${placeholders})
                ORDER BY RANDOM()
                LIMIT ?
            `).all(...existingIds, needed).map(parseFilmListRow);
        } else {
            random = db.prepare(`
                SELECT ${filmListColumns()} FROM films
                ...
                ORDER BY RANDOM()
                LIMIT ?
            `).all(needed).map(parseFilmListRow);
        }
